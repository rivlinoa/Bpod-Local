%{
%% CueInCloudBBN
Following entry the BBN entry signal is played, followed by a tone cloud (or not, with some probability), 
% in one of  attenuations logathimically spaced). 0 attenuation in the data coresponds to no cloud. 
After a random delay (default max 1 sec), a cue is played in a second speaker. 
Cue is auditory or auditory + visual with some probability.
Bpod supplies reward for the first nosepoke in the port (default 10 ul) when the cue is active.
If there is a premature nosepoke the cue will not be played, though the cloud (3 sec long) will not be stopped. 

Created by Noa 3.12.18
%}

function CueInCloudBBN
...
%             reward_supplied_plot(BpodSystem.GUIHandles.reward_supplied,'update', BpodSystem.GUIData.SubjectName, BpodSystem.Data.reward_supplied(trial_number))
    ...
    %Save this trial data to the pre-defined file name (in prepare to protocol function). 
