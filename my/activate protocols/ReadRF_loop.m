...
experiment='CueGoRig'; %can be replaced by a fitting function
load('C:\Users\Owner\Documents\Bpod Local\Data\animals_09_06_18.mat')% prepare_to_protocol_CueGo('Start', animals)
%load('C:\Users\owner\Documents\Bpod Local\Data\animals_09_06_18.mat')% prepare_to_protocol_CueInCloud('Start', animals)
...
while(1)
    ...
    if (length(tag)==12) %make sure there wasn't an error in the read (check if valid for all tags!)
        fclose(RFID);
        disp(tag)
        Ind_Settings = choose_settings_cugorig(tag);
